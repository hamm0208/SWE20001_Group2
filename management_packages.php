            ...
            <div class="col-10 ">
                <div class="row">
                    <div class="col-10 pt-5">
                        <h1 class="pt-5 fira-sans-black">Manage Packages</h1>
                    <button class='playfair-display management_btn mt-4'  onclick="location.href='management_addPackage.php';">Add new package</button>

                    </div>
                </div>
                <div class="container-fluid inventory_container">
                    <form action="management_packages.php" method="GET">
                        <div class="row">
                            <div class="col-4">
                                <label for="name" class='fira-sans-black'>Package Name:</label>
                                <input type="text" name='name' placeholder="Search for package name" class="w-75" value='<?php echo isset($_GET['name'])? $name: ""?>'>
                            </div>
                            <div class="col">
                                <label for="availability" class='fira-sans-black'>Availability:</label>
                                <select id="availability" name="availability" class="w-50">
                                    <option value="All" <?php echo ($_GET['availability'] ?? '') === 'All' ? 'selected' : ''; ?>>All</option>
                                    <option value="1" <?php echo ($_GET['availability'] ?? '') === '1' ? 'selected' : ''; ?>>Available</option>
                                    <option value="0" <?php echo ($_GET['availability'] ?? '') === '0' ? 'selected' : ''; ?>>Not Available</option>
                             </select>
                            </div>
                            <div class="col">
                            <button types='submit' class='w-25 management_btn'>Search</button>
                            <button type='reset' class='w-25 management_btn' onclick="window.location.href = 'management_packages.php'">Reset</button>
                            </div>
                        </div>
                    </form>
                    ...
                    <table class="table table-striped table-hover  mt-3">
                                ...
                                <th class='py-3 fira-sans-black align-middle text-center dark-header bg-dark text-light'>
                                    <?php 
                                       ...
                                       $getVariable = rtrim($getVariable, "&");
                                        echo "<a class='text-light' href=\"management_packages.php?" . $getVariable . "&ID_sort=" . ($id_sort_asc ? '0' : '1') . "\">Package ID</a>";
                                    ...
                                    <?php
                                           ...
                                           if(isset($_GET['$availability'])){
                                               if($_GET['$availability'] != ""){
                                                   $getVariable .= "availability=" . $_GET['$availability'] . "&";
                                               }
                                           }
                                          $getVariable = rtrim($getVariable, "&");
                                            
                                            echo "<a class='text-light' href=\"management_packages.php?" . $getVariable . "&inventory_sort=" . ($price_sort_asc ? '0' : '1') . "\">Price (RM)</a>";
                                                if($price_sort_asc === true){
                       ...
                       if(isset($_GET["name"])){  
                        ...
                        if(isset($_GET['availability'])){
                            $availability = $_GET['availability'];
                            if($availability !== 'All') {
                                $sql .= strpos($sql, 'WHERE') !== false ? " AND availability = '$availability'" : " WHERE availability = '$availability'";
                            }
                        }
                        if ($price_sort_asc !== null) {
                            $sql .= " ORDER BY price " . ($price_sort_asc ? "ASC" : "DESC");
                        }else{
                            if ($id_sort_asc) {
                       ...
                       $result = mysqli_query($conn, $sql);
                        if (mysqli_num_rows($result) > 0) {
                            while ($row = mysqli_fetch_array($result)) {
                                ...
                                $itemName = $row['name'];
                                $itemType = $row['availability'];
                                $itemCategory = $row['price'];
                                echo "<tr>";
                                ...
                                echo "<td class='playfair-display align-middle'>", $itemType? "Available": "Not Available", "</td>";
                                echo "<td> <a href='management_editPackage.php?package_id=" . $itemID . "'><img src='Images/web_resources/edit.png' alt='edit' class='event-logo'></a></td>";
                                echo "<td> <a href='management_deletePackage.php?package_id=" . $itemID . "'><img src='Images/web_resources/trash.png' alt='edit' class='event-logo'></a></td>";
                                echo "</tr>";
                            }
                        }
                        ?>
